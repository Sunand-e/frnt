  ...
  return (
    ...
    >
      <div
        ...
        tabIndex={0}
      >
        <div className="min-w-0 flex-1 flex items-center group" onClick={onSelect}>
          <Link href={`/admin/courses/edit?id=${courseId}&cid=${item.id}`}>
            <a className={`flex py-1 space-x-2 text-main-dark`}>
              ...
              <span className="min-w-0 flex-1 text-sm font-medium text-main-dark break-words">
          ...
          <div className="ml-auto h-7 flex space-x-2 hidden group-hover:block">
            <Trash className={`w-4 cursor-pointer`} onClick={handleClickDelete}/>
          </div>
        </div>
      </div>
    </li>
