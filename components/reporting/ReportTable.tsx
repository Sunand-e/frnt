...
const ReportTable = ({
      ...
      if(reportItemType==='user') {
        ...
        if(groupId) {
          data = data?.filter(item => {
            return item.node.groups.edges.some(groupEdge => groupEdge.node.id === groupId)
          })
        }
  ...
  const downloadCSV = () => {
    ...
    const dataRows = rows.map(row => csvCols.map(col => row.values[col.id]))
    console.log('rows')
    console.log(rows)
    exportToCsv('courseReport.csv', [headerRow,...dataRows])
  }

  return (
    <>
    <div className='flex items-center flex-col mb-2 sm:flex-row space-x-2'>
      { !!categoryFilter && <TagSelect selected={categoryId} tagType={`category`} onSelect={tag => setCategoryId(tag.id)} /> }
      { !!groupFilter && <GroupSelect selected={groupId} onSelect={group => setGroupId(group.id)} /> }
