...
const GroupForm = ({ typeName = 'group' }) => {
  ...
  const [associatedContentTabs, setAssociatedContentTabs] = useState([
  ...
  useEffect(() => {
    setFocus('name')
    if (typeName == 'organisation') {
      setActiveAssociatedContentTab('provided')

      setAssociatedContentTabs([
        { name: 'provided', title: 'Provided Content' }
      ])
    }
  }, [])
    ...
    >
      <TextInput
        ...
        inputAttrs={{
          ...
          onChange: ((e: any) => debouncedUpdate({ name: e.target.value }))
        }}
      />
      ...
      <div>
        ...
        {activeGroupUsersTab === 'members' && <GroupUsers showRoles={['Member']} />}
