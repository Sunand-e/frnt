...
const MediaLibraryItem = ({item, onItemSelect=(e)=>null}) => {
  ...
  return <>
    <div onClick={() => onItemSelect(item)} className="group block w-full aspect-video rounded-lg bg-white focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-offset-gray-100 focus-within:ring-main overflow-hidden shadow">
      { item.mediaType === 'image'
        ? <MediaImageThumb image={item} />
        : <Link href={item.location} className="aspect-video"><IconComponent className="text-main p-8  group-hover:opacity-75" /></Link>
      }
