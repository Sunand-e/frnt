...
const items = navStructure

export default function Navigation({navState}) {

  
  const router = useRouter()

  const subNav = navStructure.find(
    item => item.name === navState.topLevel
  ).subPages
  ...
  foreach navstructure Object {
    get the array index for the object
    foreach subitems object {
      if subitem.url = router.pathname {
        return [toplevelIndex, sublevelIndex]
      }
    }
    if(toplevel.url = router.pathname) {
  ...
  */
  ...
  return (
    <div
      id="sidebar"
      // className="fixed z-40 inset-0 flex-none flex h-full bg-black bg-opacity-25 w-full lg:bg-white lg:static lg:h-auto lg:overflow-y-visible lg:pt-0 lg:w-80 xl:w-80 lg:block hidden"
      className="fixed z-40 inset-0 flex-none flex h-full bg-black bg-opacity-25 w-full lg:bg-white lg:static lg:h-auto lg:overflow-y-visible lg:pt-0 lg:w-80 xl:w-80"
    >
      <div 
        id="navWrapper"
        ...
        >
          <ul>
            ...
            { items.map((item, index) => {
              ...
              if (navState.topLevel === item.name) {
